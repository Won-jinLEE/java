	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 99999; i++) {
			...
			for (int j = 0; j < saveBoxCount; j++) {
				System.out.print(j + " : ");
//				saveBox[j].info(); //1. 객체의 메소드를 통해서 접근(자바에서 전통적인 방식)
//				System.out.println(saveBox[j].name + " (잔액 :  "+saveBox[j].coin + ")"); //2. 프로퍼티에 직접 접근(권장하진 않지만 효율성을 올릴 수 있으면 이걸 사용해도 무방함)
				System.out.println(saveBox[j]);// 3. toString을 통해서 알아서 불러와짐
			}
			System.out.println();
